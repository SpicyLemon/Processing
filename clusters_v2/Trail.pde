class Point {
  ...
  Point Accelerate(float ddx, float ddy) {
    ...
    if (this.MaxSpeed > 0) {
      this.Dx = minMax(this.Dx, this.MaxSpeed);
      this.Dy = minMax(this.Dy, this.MaxSpeed);
    }
    return this;
  }
  ...
  boolean IsZero() {
    return isZero(this.X) && isZero(this.Y) && isZero(this.Dx) && isZero(this.Dy);
  }
}
...
class Trail {
  ...
  Trail Draw() {
    ...
    for (int i = 0; i < this.Points.length-1; i++) {
      // Stroke Weight = 4 on first segment, down to 1 on last.
      // (length, 4) - (0, 1)
      // m = (4-1)/(length-0) = 3/length
      // b = 1
      strokeWeight(1.0+3.0/this.Points.length*(float)i);
    ...
    circle(this.Points[fi].X, this.Points[fi].Y, 3);
