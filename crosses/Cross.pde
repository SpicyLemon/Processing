...
class Cross {
  ...
  Cross WithPalette(Palette pal) {
    this.Pal = pal;
    this.Hist = new CrossHist[pal.Size()];
    this.HistIndex = -1;
    return this;
  }
  ...
  Cross Draw() {
    ...
    for (int i = 0; i < this.Pal.Size(); i++) {
      int j = (i + this.HistIndex+1) % this.Hist.length;
      CrossHist h = this.Hist[j];
      if (h == null) {
        continue;
      }
      stroke(this.Pal.Get(this.Pal.Size()-1-i).Value);
      line(h.L1p1.X, h.L1p1.Y, h.L1p2.X, h.L1p2.Y);
      line(h.L2p1.X, h.L2p1.Y, h.L2p2.X, h.L2p2.Y);
    }
    return this;
  }
  ...
  Cross DrawHist(int i) {
    ...
    int c = this.Pal.Size()-1-i;
    if (c == 0) {
      // If we're on color 0, just draw the cross.
      stroke(this.Pal.Get(c).Value);
    ...
    if (h2 != null) {
      noStroke();
      fill(this.Pal.Get(c).Value);
      quad(h1.L1p1.X, h1.L1p1.Y, 
           h1.L1p2.X, h1.L1p2.Y,
           h2.L1p2.X, h2.L1p2.Y,
           h2.L1p1.X, h2.L1p1.Y);
      quad(h1.L2p1.X, h1.L2p1.Y, 
           h1.L2p2.X, h1.L2p2.Y,
           h2.L2p2.X, h2.L2p2.Y,
           h2.L2p1.X, h2.L2p1.Y);
    }
