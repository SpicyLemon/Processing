...
class Tracer {
  ...
  Tracer WithColor(color col) {
    this.Col = col;
    this.histPal = new Palette(this.history.length, #FFFFFF, this.Col);
    return this;
  }
  ...
  Tracer DrawI(int i) {
    ...
    pg.stroke(this.histPal.Get(i+1).Value);
    pg.strokeWeight(this.strokeMult * (float)i + 1.0);
  ...
  Tracer DrawDot() {
    if (this.Size > 0.0001) {
      ...
      pg.fill(this.Col);
      Spot cur = this.Cur();
      pg.circle(cur.X, cur.Y, this.Size);
    }
